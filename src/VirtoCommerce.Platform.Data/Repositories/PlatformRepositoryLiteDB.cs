...
{
    public class PlatformRepositoryLiteDB : LiteDBRepositoryBase, IPlatformRepository
    {
        public PlatformRepositoryLiteDB(ILiteDbContext dbContext)
            : base(dbContext)
        {
        }

        #region IPlatformRepository Members
        public virtual IQueryable<SettingEntity> Settings
            => DbContext.Database.GetCollection<SettingEntity>().FindAll().AsQueryable();

        public virtual IQueryable<DynamicPropertyEntity> DynamicProperties
            => DbContext.Database.GetCollection<DynamicPropertyEntity>().FindAll().AsQueryable();

        public virtual IQueryable<DynamicPropertyDictionaryItemEntity> DynamicPropertyDictionaryItems
            => DbContext.Database.GetCollection<DynamicPropertyDictionaryItemEntity>().FindAll().AsQueryable();


        public virtual IQueryable<OperationLogEntity> OperationLogs
            => DbContext.Database.GetCollection<OperationLogEntity>().FindAll().AsQueryable();
        ...
        public IQueryable<AssetEntryEntity> AssetEntries
            => DbContext.Database.GetCollection<AssetEntryEntity>().FindAll().AsQueryable();
